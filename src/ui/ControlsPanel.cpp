...
// "Dimensions" is edited in real frame pixels (X = width, Y = height), like
// Position — converted to/from LayerTransform's scalePct (X) and aspectPct
// (the extra Y stretch on top of it) against the layer's native size.
constexpr int kDimMinPx = 1;
constexpr int kDimMaxPx = 100000;
    ...
    : QWidget(parent)
{
    ...
    QWidget* box = nullptr;
    {
        ...
        // ── Dimensions (W / H in frame px) — same twin-box row as Position,
        // so the non-uniform size the canvas edge drags produce (aspectPct)
        // is editable numerically too. ──
        {
            auto* grp = new QVBoxLayout;
            grp->setContentsMargins(0, 0, 0, 0);
            grp->setSpacing(Ui::px(Ui::kGapLabelToCtrl));
            { auto* l = makeParamLabel("Dimensions"); grp->addWidget(l); }
            auto* dimRow = new QHBoxLayout;
            dimRow->setContentsMargins(0, 0, 0, 0);
            dimRow->setSpacing(Ui::px(Ui::kGapTwinBoxes));
            m_tfDimX = new DragSpinBox("", kDimMinPx, kDimMaxPx, 0);
...
void ControlsPanel::setTransform(const LayerTransform& t, QSize nativeSize)
{
    ...
    // last known scale so an edit elsewhere can't wipe it.
    m_curSrcW = nativeSize.width();
    m_curSrcH = nativeSize.height();
    m_curScalePct  = t.scalePct;
    m_curAspectPct = t.aspectPct;
    ...
    setDimensions(t.scalePct, t.aspectPct);
...
void ControlsPanel::setAnimatedParams(const QSet<ParamId>& ids)
{
    ...
    // Only the X box maps to TfScale (the animatable field); Y also carries
    // aspectPct, which has no keyframe track.
    if (m_tfDimX) m_tfDimX->setAnimated(ids.contains(ParamId::TfScale));
}
...
QHash<QWidget*, ParamId> ControlsPanel::paramWidgets() const
{
    ...
    if (m_tfDimX) m.insert(m_tfDimX, ParamId::TfScale);
    return m;
}

void ControlsPanel::setDimensions(float scalePct, float aspectPct)
{
    if (!m_tfDimX || m_curSrcW <= 0 || m_curSrcH <= 0) return;
    const bool wasUpdating = m_updating;
    m_updating = true;
    const double sx = double(scalePct) / 100.0;
    m_tfDimX->setValue(qRound(m_curSrcW * sx));
    m_tfDimY->setValue(qRound(m_curSrcH * sx * double(aspectPct) / 100.0));
